...
def divideHugeNumbers(number, divisor):
    ans = "";
    idx = 0;
...
def index(request):
    ...
    for i in range(101):
        if i == 20 or i == 40 or i == 60 or i == 80 or i == 100:
            ...
            list.append(requests.get(api).json())
            api = 'https://api.etherscan.io/api?module=account&action=balancemulti&address='
        if i < 100:
            api += addresses[i] + ','
    for i in list:
        for address in i['result']:
            ether = divideHugeNumbers(divideHugeNumbers(divideHugeNumbers(address['balance'], 1000000), 1000000), 1000000)
